...
```{r}
mutation_profile = read.csv("uterine_snv_obp_data.csv", header = TRUE, row.names = 1)
sample_ids <- c(rownames(mutation_profile), "sample")
```

```{r}
copy_number_data <- read.csv("uterine_cna_obp_data.csv", header = T, row.names = 1)
```

```{r}
#count_data <- fread("TCGA-OV_TCGA_rna_count_data.csv", select = sample_ids)
count_data <- read.csv("TCGA-UCEC_TCGA_rna_count_data.csv", header = TRUE, row.names = 1)
```
...
```
```{r}
write.csv(CIBRA_res$CIBRA_results, "cibra_results_uterine.csv")
write.csv(CIBRA_res$pvalue_results, "pvalue_uterine.csv")
write.csv(CIBRA_res$fc_results, "fc_uterine.csv")
write.csv(CIBRA_res$padj_results, "padj_uterine.csv")
```

```{r}
# calculate the pvalue by comparing to a reference distribution generated with DESeq2
perm_data <- read.csv("TCGA-UCEC_1000_sample_permutations.csv", header = T, row.names = 1)
...
write.csv(CIBRA_res_stat$results, "cibra_results_uterine.csv")
```

```{r}
# read cibra results
CIBRA_results <- read.csv("cibra_results_uterine.csv")
```
...
print(p)
pdf("cibra_figure_uterine_OBP2A.pdf")
print(p)
dev.off()
png("cibra_figure_uterine_OBP2A.png")
print(p)
dev.off()
```
...
print(p)
pdf("cibra_figure_uterine_OBP2B.pdf")
print(p)
dev.off()
png("cibra_figure_uterine_OBP2B.png")
print(p)
dev.off()
```
