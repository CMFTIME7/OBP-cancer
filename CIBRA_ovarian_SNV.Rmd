...
```{r}
# prepare databraf
goi = c("OBP2A", "MARCH1") # check if you have both genes in the data and change if you only have one
```
...
columns = c("OBP2A") # change to only the genes that you have
```


```{r}
control_definition = "WT"
confidence = 0.1
iterations = 0
```
...
```
```{r}
write.csv(CIBRA_res$CIBRA_results, "cibra_results_ovarian_mifen_snv.csv")
write.csv(CIBRA_res$pvalue_results, "pvalue_ovarian_mifen_snv.csv")
write.csv(CIBRA_res$fc_results, "fc_ovarian_mifen_snv.csv")
write.csv(CIBRA_res$padj_results, "padj_ovarian_mifen_snv.csv")
```
...
write.csv(CIBRA_res_stat$results, "cibra_results_ovarian_mifen_snv.csv")
```
...
print(p)
pdf("cibra_figure_ovarian_snv.pdf")
print(p)
dev.off()
```
...
obp2b_pvalue <- CIBRA_res$padj_results["OBP2B",]

# make a table (dataframe) from these results
df_fc_prostate <- read.csv("fc_ovarian_mifen.csv", header = TRUE, row.names = 1)
df_pvalue_prostate <- read.csv("padj_ovarian_mifen.csv", header = TRUE, row.names = 1)
write.csv(df_fc_prostate, "fc_ovarian_mifen_saved.csv", row.names = TRUE)
write.csv(df_pvalue_prostate, "padj_ovarian_mifen.csv", row.names = TRUE)
```
