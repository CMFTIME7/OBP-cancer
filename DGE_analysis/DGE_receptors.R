...
setParameters <- function(cancer_type) {
  switch(cancer_type,
         ...
         "Testis cancer" = list(
           paraStudy = "GTEX",
           paraPrimarySiteGTEx = "Testis",
           paraPrimaryTissueGTEx = "Testis",
           paraCohort = "TCGA Testicular Cancer",
           paraPrimarySiteTCGA = "Testis",
           paraDatasets = "TCGA.TGCT.sampleMap/TGCT_clinicalMatrix",
           markerGenes = NA,
           excludeSamples = NA,
           paraxlim = c(-8, 8),
           paraylim = c(0, 100)
...
# Step 5-a (the number of genes are not correct)
...
exprFinal = exprPC[!(duplicated(exprPC$gene) |
                       duplicated(exprPC$gene, fromLast = T))]
...
or_genes = c('OR2T6','OR2T6','OR2T6','OR51E2')

hormone_receptors = c('ESR1','ESR2','PGR','AR')

keyvals = ifelse(dataExpr$X %in% lipocalins, 
                 "black", 
                 ifelse(dataExpr$X %in% or_genes, 'yellow',
                        ifelse(dataExpr$X %in% hormone_receptors, 'green',
...
names(keyvals)[keyvals == "yellow"] = "Olfactory Receptors"
names(keyvals)[keyvals == "green"] = "Hormone Receptors"


keyvals.shape = ifelse(dataExpr$X == "OBP2A", 18, 
                       ifelse(dataExpr$X == "OBP2B", 17, 
                              ifelse(dataExpr$X == "LCN2", 15, 
                                     ifelse(dataExpr$X %in% or_genes, 14, 
                                            ifelse(dataExpr$X %in% hormone_receptors, 13, 
                                     ifelse(dataExpr$X %in% params$markerGenes,
                                            16,
                                            ifelse(
...
names(keyvals.shape)[keyvals.shape == 14] = "Olfactory receptors"
names(keyvals.shape)[keyvals.shape == 13] = "Hormone receptors"
...
p = EnhancedVolcano(
  ...
  ylim = params$paraylim,
  xlab = "log2 fold change",
  ylab = "-log(FDR)",
  axisLabSize = 12,
  pCutoff = 0.05,
  FCcutoff = 0.58,
  title = NULL, # paste0(paste0("TCGA Tumor vs GTEx Normal (", cancer_type),")"),
  titleLabSize = 14,
  subtitle = NULL,
  caption = NULL,
...
p <- p + ggtitle(paste0(paste0("TCGA Tumor vs GTEx Normal (", cancer_type),")")) +
  theme(
