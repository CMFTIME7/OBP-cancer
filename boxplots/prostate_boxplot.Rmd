...
```{r}
mutation_profile = read.csv("prostate_snv_obp_data.csv", header = TRUE, row.names = 1)
sample_ids <- c(rownames(mutation_profile), "sample")
```

```{r}
copy_number_data <- read.csv("prostate_cna_obp_data.csv", header = T, row.names = 1)
```

```{r}
#count_data <- fread("TCGA-prostate_TCGA_rna_count_data.csv", select = sample_ids)
count_data <- read.csv("TCGA-PRAD_TCGA_rna_count_data.csv", header = TRUE, row.names = 1)
```
...
```{r}
# Remprostatee rows with any NA in the mutation_profile data frame
...
# Remprostatee rows where the OBP2A column in mutation_profile has NA
...
design_matrix <- design_matrix %>% mutate(OBP2B = recode(OBP2B, `-1_WT` = "Loss", `2_WT` = "Amplification",  
                                        `0_WT` = "WT", `1_WT` = "Gain", `0_SNV` = "SNV"),
                         OBP2A = recode(OBP2A, `-1_` = "Loss", `0_` = "WT",
                                        `1_` = "Gain", `2_` = "Amplification"))

# remprostatee groups with less than 5 samples
...
pdf("prostate_cancer_obp_boxplot.pdf", height = 6, width = 12)
