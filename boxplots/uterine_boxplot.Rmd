...
design_matrix <- design_matrix %>% mutate(OBP2A = recode(OBP2A, `-1_WT` = "Loss",  
                                        `0_WT` = "WT", `0_SNV` = "SNV", `1_WT` = "Gain", `2_WT` = "Amplification"),
                         OBP2B = recode(OBP2B, `-1_WT` = "Loss", `0_WT` = "WT", `0_SNV` = "SNV",
...
obp2a <- ggplot(d_obp2a, aes(x=factor(OBP2A, levels = c("WT", "Loss", "Gain", "SNV")), 
                                                              y=count+1)) + 
...
pdf("uterine_cancer_obp_boxplot.pdf", height = 6, width = 12)
